...
router.post("/:charid/campaigns/add", parser.single("pictureUrl"), (req, res, next) => {
  ...
  let { title, description, dateRangeStart, dateRangeEnd, fundraisingType, agencies } = req.body;
...
// route to delete a campaign (and all its child materials)
...
router.post("/:charid/campaigns/:campid/materials/add", parser.single("pictureUrl"), (req, res, next) => {
  ...
  let { title, description, channels, mediaType, sourceUrl, dateRangeStart, dateRangeEnd } = req.body;
  if (req.file) {
    pictureUrl = req.file.url;
  } else {
    pictureUrl = req.body.pictureUrl;
  ...
  Material.create({
    ...
    .then(material => {
      // then update the corresponding charity._campaigns array with the objid of the campaign
      Campaign.findByIdAndUpdate(campid, { $push: { _materials: material._id } }).then(result => {
        res.json({
          success: true,
          material
        });
...
router.delete("/:charid/campaigns/:campid/materials/:mateid", (req, res, next) => {
  ...
  Material.findById(mateid)
    .then(removedMaterial => {
      ...
      Campaign.findByIdAndUpdate(campid, { $pull: { _materials: mongoose.Types.ObjectId(mateid) } }).then(result => {
        // delete the material
